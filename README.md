...
### v2.2.1
- 优化瀑布流布局：折叠分组后重新计算列高度，避免某列全部折叠后内容分布不均

### v2.2.0
- **分组 ID 体系**：为分组引入独立 ID，改名只改显示名称，ID 保持不变，彻底解决改名后同步丢失问题
- **可靠合并**：云端同步按分组 ID 精确匹配，跨设备改名互不冲突
- **数据迁移**：自动将旧版数据升级为新格式，无需手动操作
- **修复**：删除书签时正确记录删除标记，避免同步后被云端恢复
- **修复**：分组改名时同步更新置顶和折叠状态
- **导入导出**：支持新旧格式，导入旧数据自动创建分组
