...
### v2.1.0
- **智能追踪**：新增自动记录书签访问时间和次数（无需手动点击），实现无感式数据更新
- **动态权重排序**：重构分组排序算法，综合考虑书签数量、近期活跃度和历史点击量（得分 = 数量 + 近期活跃*2 + 点击量*0.1）
- **分组置顶**：新增分组置顶 (Pin) 功能，支持云端同步，重要分组始终触手可及
- **体验优化**：为所有卡片动作和分组动作增加了多语言悬浮提示 (Tooltip)
- **架构升级**：统一了访问时间字段名为 `lastActiveAt`，提升了同步逻辑的健壮性

### v2.0.5
- 优化了插件图标的视觉体验：未收藏页面显示为灰度图标，已收藏页面恢复为高亮图标
- 更新了插件图标的提示文字（Tooltip），悬停可直观看到点击操作（添加/取消收藏）
- 移除了原有的文字角标提示，使状态展示更加原生和清爽

### v2.0.4
- 优化登录态恢复逻辑，支持基于 refresh token 自动恢复会话
- 降低网络波动导致的误登出，初始化刷新失败时保留登录态
- 仅在 refresh token 明确失效时执行登出，减少重复登录
